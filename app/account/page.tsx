  ...
  return (
    <div className="space-y-1.5 w-full text-left">
      <div className="flex items-center justify-between">
        <label
          htmlFor={id}
          className="block text-xs font-mono uppercase tracking-wider text-[#4A4A4A] font-medium"
        >
          {label} {required && <span className="text-[#C5A059]">*</span>}
        </label>
        {error && (
          <span className="text-sm text-red-600 font-sans flex items-center gap-1">
      ...
      >
        {Icon && <Icon className="w-4 h-4 text-[#4A4A4A] mr-3 flex-shrink-0" />}
        <input
          id={id}
          type={actualType}
          value={value}
        ...
        {isPassword && (
          <button
            ...
            className="text-[#4A4A4A] hover:text-[#1F1E1D] p-1 transition-colors flex-shrink-0 ml-2"
            tabIndex={-1}
            aria-label={showPassword ? "Hide password" : "Show password"}
          >
            {showPassword ? <span className="text-base leading-none">⊘</span> : <span className="text-base leading-none">⚆</span>}
          </button>
        )}
      </div>
    </div>
      ...
      <main className="min-h-screen bg-[#FAF7F2] text-[#1F1E1D] flex flex-col justify-between">
        ...
        <div className="flex-1 pt-32 pb-20 px-4 max-w-xl mx-auto flex items-center justify-center">
          <div className="text-center space-y-3">
            ...
            <p className="text-xs font-mono uppercase tracking-widest text-[#4A4A4A]">Loading Sanctuary...</p>
          </div>
        </div>
        <Footer />
      </main>
      ...
      <div className="flex-1 pt-28 sm:pt-36 pb-20 px-4 sm:px-6 lg:px-8">
            ...
            {authMode === "signin" && (
              ...
              >
                <div className="text-center space-y-2">
                  <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#C5A059]/15 text-[#C5A059] text-sm font-mono tracking-widest uppercase font-semibold">
                    <span className="text-base leading-none">✧</span>
                  ...
                  <p className="font-serif italic text-sm sm:text-base text-[#4A4A4A] max-w-sm mx-auto leading-relaxed">
                ...
                <form noValidate onSubmit={handleSignIn} className="space-y-4">
                  ...
                  <div className="text-center pt-4 border-t border-[#DCC7AF]/30 space-y-3">
                    <p className="text-xs text-[#4A4A4A]">New to BINDY ethical slow fashion?</p>
                    <button
                      type="button"
                  ...
                  <div className="text-center pt-1">
                    <Link
                      href="/"
                      className="text-xs text-[#4A4A4A] hover:text-[#1F1E1D] inline-flex items-center gap-1.5 transition-colors"
                    >
                      <span className="text-base leading-none">←</span>
                            ...
                            <div
                              className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-mono font-medium transition-all ${
                                isDone
                                  ...
                                  : "bg-[#FAF7F2] text-[#4A4A4A] border border-[#DCC7AF]"
                              }`}
                            >
                              {isDone ? <span className="text-base leading-none">✓</span> : s.num}
                            </div>
                            <span
                              className={`text-xs font-mono tracking-wider uppercase mt-1 ${
                                isActive ? "text-[#1F1E1D] font-bold" : "text-[#4A4A4A]"
                              }`}
                            >
                              {s.label}
                            </span>
                ...
                {authMode === "signup-2" && (
                  <div className="space-y-6">
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        <label className="text-xs font-mono uppercase tracking-wider text-[#4A4A4A] font-semibold">
                          Preferred Australian Size
                        </label>
                        <button
                          type="button"
                          onClick={() => setShowSizeModal(true)}
                          className="text-sm font-mono text-[#C5A059] hover:underline flex items-center gap-1"
                        >
                          <span className="text-base leading-none">⏚</span>
                          ...
                          <button
                            ...
                            className={`py-3 px-2 rounded-2xl border text-xs font-mono text-center transition-all ${
                              prefSize === size
                                ...
                                : "border-[#DCC7AF]/60 bg-white text-[#4A4A4A] hover:border-[#1F1E1D]"
                            }`}
                          >
                            {size.replace("AU ", "")}
                          </button>
                    ...
                    {/* FAVORITE SILHOUETTE */}
                    <div className="space-y-2">
                      <label className="text-xs font-mono uppercase tracking-wider text-[#4A4A4A] font-semibold block">
                        Favorite Silhouette
                      </label>
                        ...
                        {CATEGORY_OPTIONS.map((cat) => (
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-sm text-[#4A4A4A] truncate">{cat.desc}</p>
                            </div>
                            {prefCategory === cat.category && (
                              <span className="text-base leading-none">✓</span>
                            )}
                          ...
                          <button
                            ...
                            className={`p-3 rounded-2xl border text-center transition-all ${
                              prefFit === fit.id
                                ...
                                : "border-[#DCC7AF]/60 bg-white text-[#4A4A4A] hover:border-[#C5A059]"
                            }`}
                          >
                  ...
                  <div className="space-y-5">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div className="space-y-1.5">
                        <label className="block text-xs font-mono uppercase tracking-wider text-[#4A4A4A] font-medium">
                          State / Territory
                        </label>
                        <select
                    ...
                    <div className="flex items-center gap-3 pt-3">
                      <button
                        ...
                        className="w-1/3 py-3.5 rounded-full border border-[#DCC7AF] text-xs font-mono uppercase tracking-wider text-[#4A4A4A] hover:text-[#1F1E1D]"
                      >
                        Skip for now
                      </button>
                      <button
        ...
        {authMode === "signup-complete" && account && (
          ...
          >
            ...
            <div className="space-y-2">
              <span className="text-sm font-mono uppercase tracking-[0.3em] text-[#C5A059] font-semibold">
                MEMBERSHIP CONFIRMED
              </span>
              <h1 className="font-serif text-3xl sm:text-4xl text-[#1F1E1D]">
                Welcome, {account.name}.
              </h1>
              <p className="font-serif italic text-base text-[#4A4A4A] max-w-md mx-auto leading-relaxed">
            ...
            <div className="p-6 rounded-3xl bg-gradient-to-br from-[#FAF7F2] to-[#F2ECE1] border border-[#DCC7AF]/80 text-left space-y-4 shadow-sm relative overflow-hidden">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs font-mono uppercase tracking-widest text-[#4A4A4A]">Member Tier</p>
              ...
              <div className="pt-2 border-t border-[#DCC7AF]/40 flex items-center justify-between">
                <div>
                  <p className="text-xs font-mono uppercase tracking-wider text-[#4A4A4A]">Welcome Privilege</p>
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="hidden sm:inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-[#C5A059]/15 text-[#C5A059] text-xs font-mono tracking-wider uppercase font-semibold">
                        <span className="text-base leading-none">✧</span> Origins Circle
                      </span>
                    </div>
                ...
                <div className="grid grid-cols-3 gap-3 text-center">
                  ...
                  >
                    ...
                    <span className="text-xs font-mono uppercase tracking-wider text-[#4A4A4A]">Orders</span>
                  </div>
                  ...
                  >
                    ...
                    <span className="text-xs font-mono uppercase tracking-wider text-[#4A4A4A]">Saved</span>
                  </Link>
                  ...
                  >
                    ...
                    <span className="text-xs font-mono uppercase tracking-wider text-[#4A4A4A]">Your Fit</span>
                  </div>
                </div>
              </div>
            </div>
                  ...
                  <button
                    ...
                    className={`px-5 py-3 rounded-full text-xs font-mono uppercase tracking-wider flex items-center space-x-2 whitespace-nowrap transition-all ${
                      isActive
                        ...
                        : "bg-white border border-[#DCC7AF]/60 text-[#4A4A4A] hover:border-[#1F1E1D] hover:text-[#1F1E1D]"
                    }`}
                  >
                ...
                <form onSubmit={handleSaveProfileDetails} className="space-y-6 max-w-xl">
                  ...
                  <div className="p-4 bg-[#FAF7F2] rounded-2xl border border-[#DCC7AF]/50 flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-[#4A4A4A]">Your password was set when joining Origins Circle.</p>
                    </div>
                        ...
                        >
                          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 border-b border-[#DCC7AF]/30">
                            <div>
                              ...
                              <span className="text-xs text-[#4A4A4A] ml-3">{ord.date}</span>
                            </div>
                            <div className="flex items-center space-x-3">
                              <span className="px-2.5 py-1 rounded-full text-xs font-mono uppercase tracking-wider bg-[#AFC8B1]/30 text-[#2E4A32] font-semibold">
                          ...
                          <div className="pt-2 flex flex-wrap items-center justify-between gap-2">
                            <Link
                              ...
                              className="text-xs font-mono text-[#4A4A4A] hover:text-[#C5A059] flex items-center space-x-1"
                            >
                              <span className="text-base leading-none">↻</span>
