...
export default function AccountModal({ isOpen, onClose }: AccountModalProps) {
  ...
  // Safe Guard: If not mounted or closed, return null directly so nothing exists in DOM
    ...
    >
      ...
      >
        ...
        <button
          ...
          className="absolute top-6 right-6 p-2.5 rounded-full bg-white/5 hover:bg-white/15 text-sand hover:text-white transition-colors cursor-pointer border border-white/15 z-10"
          aria-label="Close"
        ...
        {viewState === "signin" && (
          <div className="space-y-7 py-2">
            ...
            {/* Quick 1-Click Resume if account exists in storage */}
            {account && (
              <div className="p-4 rounded-2xl bg-gold/10 border border-gold/30 flex items-center justify-between gap-4 max-w-md mx-auto">
                <div className="flex items-center gap-3 min-w-0">
                  <div className="w-10 h-10 rounded-full bg-gold/20 border border-gold flex items-center justify-center text-gold font-display font-medium flex-shrink-0">
                    {getInitials(account.name)}
                  </div>
                  <div className="truncate text-left">
                    <p className="font-serif text-sm text-paper font-medium truncate">{account.name}</p>
                    <p className="text-[11px] font-sans text-sand/70 truncate">{account.email}</p>
                  </div>
                </div>
                <button
                  type="button"
                  onClick={() => setViewState("panel")}
                  className="px-4 py-2 rounded-full bg-gold text-charcoal font-sans text-[11px] uppercase tracking-wider font-bold hover:bg-paper-light transition-all flex-shrink-0 shadow-sm cursor-pointer"
                >
                  Continue
                </button>
              </div>
        ...
        {viewState === "signup-complete" && (
          <div className="py-12 text-center space-y-6 max-w-md mx-auto">
            <div className="w-16 h-16 rounded-full bg-gold/20 border-2 border-gold flex items-center justify-center mx-auto text-gold shadow-[0_0_30px_rgba(197,160,89,0.3)]">
        ...
        {viewState === "panel" && account && (
          <div className="space-y-6">
            ...
            {/* 4 Tabs with Pure CSS Active Border */}
              ...
              {[
                ...
                return (
                  <button
                    ...
                    className={`pb-3 text-xs font-sans uppercase tracking-[0.2em] font-medium transition-all duration-200 cursor-pointer relative whitespace-nowrap border-b-2 ${
                      active
                        ? "border-gold text-gold font-semibold shadow-[0_2px_10px_rgba(197,160,89,0.4)]"
                        : "border-transparent text-sand/70 hover:text-paper"
                    }`}
                  >
