...
export default function HeritageTicker() {
  ...
  return (
    <div className="relative w-full bg-ink border-y border-gold/20 py-2 sm:py-2.5 overflow-hidden select-none">
      <div className="flex w-max animate-marquee space-x-12">
        {[...items, ...items, ...items].map((text, i) => (
          <div key={i} className="flex items-center space-x-6">
            <span className="text-[10px] sm:text-[11px] font-sans tracking-[0.35em] uppercase text-sand/90 font-medium">
              {text}
            </span>
